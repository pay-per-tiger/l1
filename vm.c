...
xbinop(VM *vm, ikind op, Val v1, Val v2)
{
	...
	if(Viskind(v1, Qcval) && Viskind(v2, Qcval)) {
		cv1 = valcval(v1);
		cv2 = valcval(v2);
		switch(op) {
		case Iadd:
	...
	if(Viskind(v1, Qstr) && Viskind(v2, Qstr)) {
		s1 = valstr(v1);
		s2 = valstr(v2);
dostr:
		if(op == Iadd)
	...
	if(Viskind(v1, Qstr) && Viskind(v2, Qcval)) {
		cv2 = valcval(v2);
		if(ischarcval(cv2)) {
			char v = cvalu(cv2);
			s2 = mkstr(&v,1);
	...
	if(Viskind(v2, Qstr) && Viskind(v1, Qcval)) {
		cv1 = valcval(v1);
		if(ischarcval(cv1)) {
			char v = cvalu(cv1);
			s1 = mkstr(&v,1);
...
l1_cval(VM *vm, Imm argc, Val *argv, Val *rv)
{
	...
	if(!Viskind(argv[0], Qdom) && !Viskind(argv[0], Qns))
...
xxcast(VM *vm, Val tv, Val ov)
{
	...
	rv = 0;
	if(!Viskind(ov, Qcval) && !Viskind(ov, Qstr))
...
mknas(void)
{
	...
	mtab = mktab();
	tabput(mtab, mkvalcid(mkcid0("get")),
	       mkvalcl(mkcfn("nasget", nasbad)));
	tabput(mtab, mkvalcid(mkcid0("put")),
	       mkvalcl(mkcfn("nasput", nasbad)));
	tabput(mtab, mkvalcid(mkcid0("map")),
	       mkvalcl(mkcfn("nasmap", nasmap)));
	tabput(mtab, mkvalcid(mkcid0("ismapped")),
...
ascache1method(As *as, char *id, Closure **p)
{
	...
	*p = mkccl(id, calldispatch, 2, mkvalcl(as->dispatch), mkvalcid(cid));
}
...
nscache1method(Ns *ns, char *id, Closure **p)
{
	...
	/* bind a new caller to dispatch and hope for the best */
	*p = mkccl(id, calldispatch, 2, mkvalcl(ns->dispatch), mkvalcid(cid));
}
       ...
       Closure *lookaddr, Str *name)
{
	Tab *mtab;
	mtab = mktab();
	tabput(mtab, mkvalcid(mkcid0("looktype")), mkvalcl(looktype));
	tabput(mtab, mkvalcid(mkcid0("enumtype")), mkvalcl(enumtype));
	tabput(mtab, mkvalcid(mkcid0("looksym")), mkvalcl(looksym));
	tabput(mtab, mkvalcid(mkcid0("enumsym")), mkvalcl(enumsym));
	tabput(mtab, mkvalcid(mkcid0("lookaddr")), mkvalcl(lookaddr));
...
l1_sort(VM *vm, Imm argc, Val *argv, Val *rv)
{
	...
	if(!Viskind(argv[0], Qlist) && !Viskind(argv[0], Qvec))
...
l1_bsearch(VM *vm, Imm argc, Val *argv, Val *rv)
{
	...
	if(!Viskind(argv[1], Qlist) && !Viskind(argv[1], Qvec))
...
l1_looktype(VM *vm, Imm argc, Val *argv, Val *rv)
{
	...
	ns = 0;
	if(Viskind(argv[0], Qns))
		ns = valns(argv[0]);
	else if(Viskind(argv[0], Qdom)) {
		dom = valdom(argv[0]);
		ns = dom->ns;
	}else
		vmerr(vm,
...
l1_domof(VM *vm, Imm argc, Val *argv, Val *rv)
{
	...
	arg0 = argv[0];
	if(Viskind(arg0, Qcval)) {
		cv = valcval(arg0);
		*rv = mkvaldom(cv->dom);
	}else if(Viskind(arg0, Qstr))
		*rv = mkvaldom(litdom);
	else
		vmerr(vm,
...
{
	Cval *cv;
	if(Viskind(argv[arg], Qstr))
		return valstr(argv[arg]);
	else if(Viskind(argv[arg], Qcval)) {
		cv = valcval(argv[arg]);
		if(isstrcval(cv))
...
l1_nameof(VM *vm, Imm argc, Val *argv, Val *rv)
{
	...
	if(Viskind(argv[0], Qdom)) {
		dom = valdom(argv[0]);
		name = dom->name;
	}else if(Viskind(argv[0], Qns)) {
		ns = valns(argv[0]);
		name = ns->name;
	}else if(Viskind(argv[0], Qas)) {
		as = valas(argv[0]);
		name = as->name;
	}else
...
l1_nsptr(VM *vm, Imm argc, Val *argv, Val *rv)
{
	...
	arg0 = argv[0];
	if(!Viskind(arg0, Qns) && !Viskind(arg0, Qdom))
		vmerr(vm,
...
cntrlen(VM *vm, char *name, Val v)
{
	...
	case Qpair:
		len = 0;
		while(Viskind(v, Qpair)) {
			len++;
			p = valpair(v);
			v = p->cdr;
			if(!Viskind(v, Qpair) && !Viskind(v, Qnil))
...
l1_pop(VM *vm, Imm argc, Val *argv, Val *rv)
{
	...
	arg = argv[0];
	if(Viskind(arg, Qlist))
		listpop(vallist(argv[0]), rv);
	else if(Viskind(arg, Qtab))
...
l1_copy(VM *vm, Imm argc, Val *argv, Val *rv)
{
	...
	else if(Viskind(argv[0], Qvec))
		*rv = mkvalvec(veccopy(valvec(argv[0])));
	else if(Viskind(argv[0], Qtab))
		*rv = mkvaltab(tabcopy(valtab(argv[0])));
	else if(Viskind(argv[0], Qstr))
...
l1_concat(VM *vm, Imm argc, Val *argv, Val *rv)
{
	...
	if(Viskind(argv[0], Qlist) && Viskind(argv[1], Qlist)) {
		lst = listconcat(vm, vallist(argv[0]), vallist(argv[1]));
		*rv = mkvallist(lst);
	}else if(Viskind(argv[0], Qstr) && Viskind(argv[1], Qstr)) {
...
l1_add(VM *vm, Imm argc, Val *argv, Val *rv)
{
	...
	if(!Viskind(argv[0], Qcval) && !Viskind(argv[0], Qstr))
		vmerr(vm, "operand 1 to add must be a cvalue or string");
	if(!Viskind(argv[1], Qcval) && !Viskind(argv[1], Qstr))
...
mktopenv(void)
{
	...
	builtinns(env, "c16le", mkrootns(&c16le));
	builtinns(env, "c16be", mkrootns(&c16be));
