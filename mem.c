...
mapmem(u64 sz)
{
	...
	p = mmap(0, sz+Segsize, PROT_READ|PROT_WRITE,
...
saveheapfd(Tab *toplevel, int fd)
{
	...
	wsize = sizeof(void *);
	if(-1 == xwrite(fd, &wsize, sizeof(u8))){
		fprintf(stderr, "saveheapfd: write: %s\n", strerror(errno));
		goto fail;
	}
	sz = strlen(sysos)+1;
...
#define XRD(to, fr, sz) { 		\
	...
	XRD(&wsize, p, sizeof(u8));
	if(wsize != sizeof(void *)) {
		fprintf(stderr, "restoreheap: word size version mismatch "
			"(need %u, got %u)\n",
			(unsigned)sizeof(void *), (unsigned)wsize);
		goto fail;
	}
	XRD(&sz, p, sizeof(u32));
	if(sz > sizeof(buf)){
...
#undef XRD
	...
	return toplevel;
fail:
	if(!file && (getuid() != geteuid())) {
		fatal("failed to load bundled heap");
	}
	if(op && op != MAP_FAILED)
